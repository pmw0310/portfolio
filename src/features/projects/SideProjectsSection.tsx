  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto space-y-16">
        ...
        {/* 메인 사이드 프로젝트 1: 소녀전선2 by BlackOlf */}
        {gf2Project && (
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-10 shadow-2xl space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
              <div>
                ...
                <h3 className="text-2xl md:text-4xl font-bold font-display text-white mt-3">
                  {gf2Project.title}
                </h3>
              </div>
              <a
                ...
                className="inline-flex items-center gap-2 text-cyan-brand font-semibold hover:underline text-base"
              >
                gf2.blackolf.dev ↗
              </a>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="lg:col-span-6 space-y-4">
                {gf2Project.features.map((feature, idx) => (
                  ...
                  >
                    <div className="font-bold text-white text-base">{feature.title}</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sideProjects.map((project) => (
            <div
              key={project.id}
              className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-lg hover:border-slate-700 transition-all hover:-translate-y-1 flex flex-col"
            >
              ...
              <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                <div>
                  <div className="flex items-center justify-between text-xs text-slate-400 mb-2">
                    <span className="font-semibold text-cyan-brand uppercase tracking-wider">
                      {project.category}
                    </span>
                ...
                <div className="flex flex-wrap gap-1.5 pt-2 border-t border-slate-800">
                  {project.techStack.map((tech, idx) => (
                    <span
                      key={idx}
                      className="text-xs bg-slate-950 text-slate-400 border border-slate-800 px-2 py-0.5 rounded-md"
                    >
                      {tech}
                    </span>
                  ))}
